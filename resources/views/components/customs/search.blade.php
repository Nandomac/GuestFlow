...
- selectedId (string, opcional): ID do item selecionado
...
>
    ...
    <div class="grid grid-cols-12 gap-4">
        ...
        <div class="md:col-span-3 col-span-12" x-show="isSearchable">
            <input
                type="text"
                ...
                autocomplete="off"
            />
            <div class="text-red-500 text-sm mt-1 divErro" id="{{ $searchName }}_selected_id-error"></div>
        </div>


        <div :class="isSearchable ? 'md:col-span-8 col-span-12' : 'col-span-12'">
            <input
                type="text"
            ...
            />
            <div class="text-red-500 text-sm mt-1 divErro" id="{{ $searchName }}_selected_description-error"></div>
        </div>


        <div class="flex items-center" x-show="isSearchable">
            <div class="flex items-center">
                <button type="button"
                        @click="clearSelection"
                        class="rounded-lg bg-blue-600 px-3 py-2 text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
                >
                    Clean
                </button>
            </div>        </div>
...
<script>
    function searchComponent(apiUrl, initialValue = '', showCode = false, selectedLabelInit = '', isSearchable = true, onSelectCallbackName = null, searchName = '', selectedIdInit = '', apiURLParam = '') {
        return {
            search: initialValue,
            open: false,
            suggestions: [],
            selectedId: selectedIdInit,
            selectedDescription: selectedLabelInit,
            ...
            async fetchSuggestions() {
                ...
                try {
                    const response = await fetch(`${apiUrl}/${this.search}${apiURLParam}`, {
                        method: 'GET',
                        headers: {
